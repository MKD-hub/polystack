...
var g_print_buf: [256]u8 = undefined; // buffer to print from. TODO: Abstract this into a logger.
...
export fn getEditorConfig(fov: f32, aspect: f32, near: f32, far: f32, canvas_width: f32, canvas_height: f32) void {
    ...
    const formatted_string = std.fmt.bufPrintZ(&g_print_buf, "Editor Config: FOV={d:.2}, Aspect={d:.4}, Near={d:.4}, Far={d:.4}, CanvasW={d:.0}, CanvasH={d:.0}", .{
        g_editor_config.fov,
...
export fn updateCamera() void {
    g_camera.updateCamera(constants.world_origin);
}
